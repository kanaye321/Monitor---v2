...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/monitor-inventory/import", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Starting import of ${monitors.length} monitors...`);
      ...
      for (let i = 0; i < monitors.length; i++) {
        try {
          ...
          const newMonitor = {
            seatNumber: monitor.seatNumber.trim(),
      ...
      const response = {
        ...
        failed,
        errors,
        message: `Import completed. ${successful} monitors imported, ${failed} failed.`
      };
